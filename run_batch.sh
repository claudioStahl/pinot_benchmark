...
# Launch batch job
docker run -it --network pinot_vpcbr -v $(pwd)/config:/tmp/config -v $(pwd)/tmp/data:/tmp/data apachepinot/pinot:0.10.0 LaunchDataIngestionJob -jobSpecFile /tmp/config/transcripts-ingestionjobspec.yaml
